...
function drawChart() {
	...
	svgLeft.append("g")
		.attr('class', "med-axis-tick-mark-left")
		.attr("x", "0")
		.attr("x1", "0")
		.attr("x2", "0")
		.attr("transform", "translate(0, " + margin.top + ")")
		...
		svgLeft.selectAll(".med-axis-tick-mark-left text")
			.attr("x", "30");

		svgLeft.selectAll(".grid-left line").attr("x1", "60").attr("x2", width);
	  
	// add the graph's bars
	let bars = svgLeft.selectAll("rect").data(data).enter().append("rect");
	...
	let tags = svgLeft.selectAll(".tags-left")
		.filter('tags-left')
		.data(data).enter()
		.append("text")
		.text(function(d) {
		...
		.attr('class', 'tags-left');
	
	// add the x axis
	let xAxis = svgLeft.selectAll(".xaxis-ticks-left")
		.data(xTicks)
		.enter()
		.append("text")
		.text(function (d,i) {
	...
	let yTicksText = svgLeft.selectAll('.med-axis-tick-mark-left .tick text')
		.each(function(d) {
			if (d == 0) {
