...
const padding = { top: 15, right: 10, bottom: 25, left: 20 };
let width = 420;
let height = 270;
let textWidth = 420;
...
// set the point center
function pointLen(point, i) {
    let lenRatio = 0;
    ...
    console.log(parseInt(lenRatio));
    return parseInt(lenRatio);
}
...
<div class="chart" bind:clientWidth={width} bind:clientHeight={height}>
	...
	<g class="axis y-axis" transform="translate(0,{padding.top})">
		{#each yTicks as tick, i}
			<g class="tick tick-{tick}" transform="translate(10, {yScale(tick) - padding.bottom + 10})">
				<line x1="35px" x2="{innerWidth}"></line>
				<text dx="10px" y="3">{tick >= 100 ? formatTick(tick) : tick}</text>
			</g>
		{/each}
	</g>
