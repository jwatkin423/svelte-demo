...
function drawChart() {
	...
	d3.selectAll("#chart-right > *").remove();
	...
	// svg for d3
	let svgRight = d3.select(".median-chart-right")
	...
	svgRight.append("g")
		.attr('class', "med-axis-tick-mark-right")
		.attr("x", "0")
		.attr("x1", "0")
		.attr("x2", "0")
		.attr("transform", "translate(0, " + margin.top + ")")
		...
		svgRight.selectAll(".med-axis-tick-mark-right text")
			.attr("x", "30");

		svgRight.selectAll(".grid-right line").attr("x1", "60").attr("x2", width);
	  

	let bars = svgRight.selectAll("rect").data(dataTwo).enter().append("rect");
	...
	let tags = svgRight.selectAll('.tags-right')
		.data(dataTwo)
		.enter()
		.append("text")
		.text(function(d) {
		...
		.attr('class', 'tags-right');

	// add the x axis
	let xAxis = svgRight.selectAll('.xaxis-ticks-right')
		.data(xTicks)
		.enter()
		.append("text")
		.text(function (d,i) {
	...
	let yTicksText = svgRight.selectAll('.med-axis-tick-mark-right .tick text')
		.each(function(d) {
			if (d == 0) {
...
<div class="chart-right">
    <div class="chart">
        <svg class="median-chart-right" id='chart-right'></svg>
    </div>
</div>